...
export function BentoGrid({ playlists, isLoading, folderName, editMode, onEditPlaylist }: BentoGridProps) {
  return (
    <div>
      {folderName && !isLoading && (
        <div className="mb-4">
          <p className="text-sm text-gray-500">
            Showing playlists in <span className="font-semibold text-gray-700">{folderName}</span>
          </p>
        </div>
      )}
        ...
        {isLoading
          ...
          : playlists.length > 0 ? (
              ...
              <div className="col-span-full flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-sm text-gray-500">
                  {folderName ? `Add playlists to ${folderName}` : "Try selecting a different folder"}
                </p>
              </div>
            )}
      </div>
